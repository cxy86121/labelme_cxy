...
## Changes Log
### 2021.12.29
添加类似魔改 labelImg 的txt记录功能. 按数字键 `1~9` 会将当前标签文件记录到对应数字的 txt 中,txt 位于数据文件夹下,数字 `0` 会将当前文件从所有记录txt 中去除.操作消息显示在状态栏中.
**注意事项**:
1. 事件是写在画布控件上的,因此使用前需要使用画布控件拥有当前焦点,若按了没效果,注意用鼠标点一下画布控件,使之获得焦点.
2. 数字键 `0` 删除会将当前文件从所有记录 txt 中删除.
3. 当软件开启时,手动更改记录 txt,可能会产生意料之外的错误,因为程序中维护的信息仅仅在中间变量为空时才同步一次,保持和txt信息一致

**TODO**
- [ ] 目标是记录的时候就写入,且和主流程在同一线程,导致文件需要多次开关,比较蠢,先简单写了
